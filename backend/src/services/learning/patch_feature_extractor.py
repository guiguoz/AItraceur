"""
patch_feature_extractor.py — Extraction des 18 features visuelles ISOM pour patches de carte CO.
...
Feature space (18 dimensions) :
   ...
   [17]    is_urban             — 1 si coordonnées dans zone urbaine dense, 0 sinon
...
# 18 features : 7 ISOM global + 7 ISOM centre + 3 géométriques + 1 contexte urbain
FEATURE_NAMES: list[str] = [
    ...
    # Contexte (1)
    "is_urban",  # 1 si en zone urbaine dense, 0 sinon
]
...
def extract_features(
    ...
    return np.concatenate(
        [
            global_feats,
            center_feats,
            [edge_density, corner_density, entropy, is_urban],
        ]
    ).astype(np.float32)
